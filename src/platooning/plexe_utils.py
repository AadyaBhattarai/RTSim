...
def add_vehicle(plexe, vid, position, lane, speed, vtype=None):  #made changes to this 
    if plexe.version[0] >= 1:
        ...
        if vid == "v.0.0":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto1")
            
        if vid == "v.0.1":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto2")
            
        if vid == "v.0.2":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto3")
            
        if vid == "v.0.3":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto4")
            
        if vid == "v.0.4":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto5")
            
        if vid == "v.0.5":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto6")
            
        if vid == "v.0.6":
            traci.vehicle.add(vid, "platoon_route", pos=position, speed=speed,
                          lane=lane, typeID="vtypeauto7")
            

def add_platooning_vehicle(plexe, vid, position, lane, speed, tau, cacc_spacing,
                            vtype=None, real_engine=False):
    """
...
# def start_sumo(config_file, already_running,gui=True,port=None):
#     """
...
#     """
#     arguments = ["--lanechange.duration", "3", "--emission-output.step-scaled","true","-c"]
#     # arguments = ["--lanechange.duration", "3", "-c"]
#     if port is not None:
#         arguments.extend(["--remote-port", str(port)])
#     # finally point at the config file
#     sumo_cmd = [sumolib.checkBinary('sumo-gui' if gui else 'sumo')]
#     arguments.append(config_file)
    
#     if already_running:
#         traci.load(arguments, port=port)
#     else:
